...
- 2026-09-11 · Daniela · EXECUÇÃO F1-T04: migration, rotas server-side, autenticação, serviços e UI mínima implementados; QA oficial 0.0.5 passou, mas smoke test falhou em `pendencia_validacao: cannot be blank`.
- 2026-09-11 · Daniela · DEBUG F1-T04: booleano `pendencia_validacao` obrigatório rejeitava `false` como blank → migration 0002 tornou o campo opcional; QA 0.0.6 passou; criação, idempotência e pendência de validação passaram no preview; correção append-only ainda não comprovada, task permanece em correção.
